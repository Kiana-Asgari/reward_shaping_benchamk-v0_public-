...
def get_policy_given_theta(self, theta, axis=-1):
...
class REINFORCE():
    ...
    def learn(self, total_episodes, log_interval, log_num, env,
                    learning_rate, gamma):
        for n_epi in range(total_episodes):
            reward_buffer = []
            state_buffer = []
            action_buffer = []
            self.run_single_episode(env, reward_buffer, state_buffer, action_buffer)
            # print('[episode]', n_epi)
            # print('     states: ', state_buffer)
            # print('     policy', self.parameter_policy)
            self.backward(reward_buffer, state_buffer, action_buffer, learning_rate, gamma)
